...
- XHS 卡片图链路：长文案 -> AI 规划多张卡片 -> 多次调用图片模型生成 -> 上传 OSS。
...
- 每一张卡片会单独构造图片提示词，并调用 Agnes 图片模型生成。
- 每张生成图会逐张上传到 OSS，并把请求、规划结果、每张生图请求和 OSS 链接写入 SQLite。
...
- 已验证文本模型规划、图片生成、OSS 上传、SQLite 入库、状态查询链路。
- 长文案多图卡片测试已通过。
- 测试 requestId：`f387810d-7fb7-47a0-89e2-63198234516e`
- 已验证薪资期望长文案可以自动拆成封面、内容页和结尾页，并逐张上传 OSS。
- 图片模型多次生成卡片测试已通过。
- 测试 requestId：`a216a4d1-9d20-40a0-8c8d-e711e17d824c`
- 已验证 Card Image 会按卡片数量多次调用 Agnes 图片模型，生成 PNG 后上传 OSS。
